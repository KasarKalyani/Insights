...
const parser = StructuredOutputParser.fromZodSchema(
  z.object({
    ...
    negative: z
      .string()
      .describe(
        'is journalentry negative? does it contains negative emotions?'
      ),
    color: z
      .string()
      .describe(
        'a hexadecimal color code to represent emaotions. (i.e, #0101fe represent blur fot happiness)'
      ),
  })
)
...
const getPrompt = async (content) => {
  ...
  const prompt = new PromptTemplate({
    template:
      'analyze following journal entry. follow the instructions and format your response to match instructions given, no matter what! \n{format_instructions}\n{entry}',
